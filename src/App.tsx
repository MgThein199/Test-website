...
export default function App() {
  ...
  const [adminSection, setAdminSection] = useState<'details' | 'content'>('details');
  ...
  const isAdmin = useMemo(() => {
    // Only the user provided in metadata and current session is technically the admin
    const adminEmails = ['minnnaing.2020.mm@gmail.com'];
    ...
    return (
      <div className="flex flex-col">
        ...
        >
          ...
          {user && (isAdmin || node.author_id === user.uid) && (
            <button 
    ...
    <div className="flex h-screen bg-app-bg text-app-text-primary font-sans transition-colors duration-300">
      ...
      `}>
          ...
          {showAdmin && user ? (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-[10px] uppercase tracking-[0.2em] text-app-text-primary font-bold">New Archives Entry</h3>
                <button onClick={() => setShowAdmin(false)} className="text-app-text-secondary hover:text-app-text-primary">
                  <X size={14} />
                </button>
              </div>
              ...
              <form onSubmit={handleCreateNode} className="space-y-4">
                ...
                <div className="border border-app-border">
                  <button 
                    type="button"
                    onClick={() => setAdminSection(adminSection === 'details' ? 'content' : 'details')}
                    className="w-full flex items-center justify-between p-3 bg-app-bg text-[10px] uppercase tracking-widest font-bold"
                  >
                    1. Node Details
                    {adminSection === 'details' ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                  </button>
                  ...
                  <AnimatePresence mode="wait">
                    {adminSection === 'details' && (
                      <motion.div 
                        ...
                        animate={{ height: 'auto' }}
                        exit={{ height: 0 }}
                        className="overflow-hidden border-t border-app-border p-4 space-y-3"
                      >
                        <div>
                          <label className="block text-[9px] text-app-text-secondary uppercase mb-1">Title</label>
                          <input required type="text" value={form.title} onChange={e => setForm({...form, title: e.target.value})} className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary" />
                        </div>
                        <div>
                          <label className="block text-[9px] text-app-text-secondary uppercase mb-1">Slug</label>
                          <input required type="text" value={form.slug} onChange={e => setForm({...form, slug: e.target.value})} className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary font-mono" />
                        </div>
                        <div className="grid grid-cols-2 gap-2">
                          <div>
                            <label className="block text-[9px] text-app-text-secondary uppercase mb-1">Level</label>
                            <input required type="number" value={form.level} onChange={e => setForm({...form, level: parseInt(e.target.value)})} className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary" />
                          </div>
                          <div>
                            <label className="block text-[9px] text-app-text-secondary uppercase mb-1">Order</label>
                            <input required type="number" value={form.order_index} onChange={e => setForm({...form, order_index: parseInt(e.target.value)})} className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary" />
                          </div>
                        </div>
                        <div>
                          <label className="block text-[9px] text-app-text-secondary uppercase mb-1">Parent</label>
                          <select value={form.parent_id} onChange={e => setForm({...form, parent_id: e.target.value})} className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary">
                ...
                <div className="border border-app-border">
                  <button 
                    type="button"
                    onClick={() => setAdminSection(adminSection === 'content' ? 'details' : 'content')}
                    className="w-full flex items-center justify-between p-3 bg-app-bg text-[10px] uppercase tracking-widest font-bold"
                  >
                    2. Markdown Content
                    {adminSection === 'content' ? <ChevronDown size={12} /> : <ChevronRight size={12} />}
                  </button>
                    ...
                    {adminSection === 'content' && (
                      ...
                      >
                        <div className="flex items-center space-x-1 mb-2">
                          <button type="button" onClick={() => insertMarkdown('**', '**')} className="p-1 border border-app-border hover:bg-app-bg text-app-text-secondary"><Bold size={10} /></button>
                          <button type="button" onClick={() => insertMarkdown('_', '_')} className="p-1 border border-app-border hover:bg-app-bg text-app-text-secondary"><Italic size={10} /></button>
                          <button type="button" onClick={() => insertMarkdown('`', '`')} className="p-1 border border-app-border hover:bg-app-bg text-app-text-secondary"><Code size={10} /></button>
                          <button type="button" onClick={() => insertMarkdown('> ')} className="p-1 border border-app-border hover:bg-app-bg text-app-text-secondary"><Quote size={10} /></button>
                        </div>
                        <textarea 
                          ref={contentRef}
                          required 
                          ...
                          className="w-full bg-app-sidebar border border-app-border p-2 text-xs outline-none focus:border-app-text-primary font-serif resize-none"
                        />
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <button type="submit" className="w-full py-3 bg-app-text-primary text-app-bg text-[10px] uppercase tracking-[0.2em] font-bold hover:opacity-90 shadow-lg">
                  Commit Change
                </button>
              </form>
            </div>
        ...
        <div className="p-6 bg-app-bg border-t border-app-border space-y-4">
          {user ? (
            <>
              <div className="px-1 py-1 mb-2 bg-app-surface border border-app-border flex items-center justify-between">
                <div className="flex items-center space-x-2 px-2">
                  <div className="w-2 h-2 rounded-full bg-green-500"></div>
                  <span className="text-[9px] uppercase tracking-widest text-app-text-secondary font-bold truncate max-w-[120px]">
                    {user.email}
                  </span>
                </div>
                <button 
                  onClick={logout}
                  className="p-2 text-app-text-secondary hover:text-red-500 transition-colors"
                  title="Logout"
                >
                  <LogOut size={12} />
                </button>
            ...
            <button 
              ...
              className="w-full py-2 border border-app-text-primary text-[10px] uppercase tracking-widest font-bold hover:bg-app-text-primary hover:text-app-bg transition-colors bg-app-surface shadow-sm disabled:opacity-50"
            >
              {isLoggingIn ? 'Connecting...' : 'Admin Access'}
            </button>
          )}
          <div className="pt-2 border-t border-app-border/50">
            <div className="text-[9px] text-app-text-secondary uppercase tracking-[0.2em]">User Role</div>
            <div className={`text-[9px] font-bold uppercase tracking-widest ${isAdmin ? 'text-green-600' : 'text-amber-600'}`}>
      ...
      {/* Admin Panel Overlay REMOVED - now in sidebar */}
    </div>
  );
}
